...
tenant_mismatch_code="$(command curl -sS -o tenant-mismatch.txt -w '%{http_code}' -X POST "$BASE_URL/api/v1/ai/chat" \
  -H 'Content-Type: application/json' \
...
unsupported_code="$(command curl -sS -o unsupported-response.txt -w '%{http_code}' -X POST "$BASE_URL/api/v1/ai/chat" \
  -H 'Content-Type: application/json' \
  -H "Authorization: Bearer $token" \
  -H 'X-Tenant-Id: 1' \
